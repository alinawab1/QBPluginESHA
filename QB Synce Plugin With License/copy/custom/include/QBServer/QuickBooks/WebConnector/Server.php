...
//			$input = file_get_contents('php://input');
	...
	protected function _adapterFactory($adapter, $wsdl, $soap_options, $loglevel)
	{
		...
		$class['index'] = 'QuickBooks_Adapter_Server_' . $adapter;
		
		QuickBooks_Loader::load($file);
		
		if (class_exists($class['index']))
		{
			return new $class['index']($wsdl, $soap_options);
		}
	...
	public function handle($return = false, $debug = false)
	{
		...
		$GLOBALS['log']->fatal("In Server::handle()");

		// 
		if ($_SERVER['REQUEST_METHOD'] == 'POST')
		{
			$GLOBALS['log']->fatal("In Server::handle() POST");
			$this->_headers();
		...
		{
			if ($contents = sugar_file_get_contents($this->_wsdl))
			{
				$this->_headers();
				print($contents);
				exit;
