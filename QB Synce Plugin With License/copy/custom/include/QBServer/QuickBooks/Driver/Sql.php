	...
	*/
	...
	public function query($sql, &$errnum, &$errmsg, $offset = 0, $limit = null, $vars = array())
	{
			...
			and count($vars))
		{
			...
			$tmp['func'] = 'sprintf';
			$GLOBALS['log']->fatal(print_r($vars, 1));
//			if(!empty($vars) && count($vars) > 0) {
//				$tmp['param'] = '"'.implode(", ", $vars).'"';
//				$sql = $tmp['func']($tmp['param']);
//			} else {
//				$sql = $tmp['func']();
//			}
//			$sql = call_user_func_array('sprintf', $vars);
	 ...
	 * @return boolean
	 */
	public abstract function _rewind($res);
...
//			while (false !== ($file = readdir($dh)))
			...
			{
				if ($file{0} == '.' or sugar_is_dir($defaults['quickbooks_sql_schema'] . '/' . $file))
				{
					continue;
				}
				
				$xml = sugar_file_get_contents($defaults['quickbooks_sql_schema'] . '/' . $file);
	 ...
	 * @return object
	 */
	public function getD($table, $restrict)
	{
		$sql = '';
		$where = array();
