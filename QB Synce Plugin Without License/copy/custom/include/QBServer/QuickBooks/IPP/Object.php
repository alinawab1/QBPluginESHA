...
class QuickBooks_IPP_Object
{
	...
	public function getDateType($field, $format = 'Y-m-d')
	{
		...
		$value = str_replace('T00:00:00Z', '', $this->getD($field));
		...
		foreach ($this->_data as $key => $value)
		{
			...
			else if (is_array($value))
			{
				foreach ($value as $skey => $svalue)
				{
					...
					{
						$tmp['func'] = $callback;
						$this->_data[$key][$skey] = $tmp['func']($this->resource(), $key, $svalue);
//						$this->_data[$key][$skey] = call_user_func($callback, $this->resource(), $key, $svalue);
			...
			{
				$tmp['func'] = $callback;
				$this->_data[$key] = $tmp['func']($this->resource(), $key, $value);
