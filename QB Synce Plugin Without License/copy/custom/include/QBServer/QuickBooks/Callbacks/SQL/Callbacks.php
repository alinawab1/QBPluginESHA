...
public static function InventoryAssemblyLevelsRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	...
	public static function ListDelRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if ($arr = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . strtolower($extra['objectType']), array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<ListDelRq requestID="' . $requestID . '">
						...
						<ListID>' . $Object->getD('ListID') . '</ListID>
					</ListDelRq>
				</QBXMLMsgsRq>
			</QBXML>';
	...
	public static function TxnVoidRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if ($arr = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . strtolower($extra['object']), array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
			$xml = '';
			$xml .= '<?xml version="1.0" encoding="utf-8"?>
			<?qbxml version="' . QuickBooks_Callbacks_SQL_Callbacks::_version($version, $locale) . '"?>
	...
	public static function InvoiceDeriveRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		else if ($arr = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'invoice', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
			$xml = '';
			$xml .= '<?xml version="1.0" encoding="utf-8"?>
				<?qbxml version="' . $version . '"?>
		...
		foreach ($List->children() as $Node)
		{
			switch ($type)
			{
				...
				case QUICKBOOKS_OBJECT_INVOICE:
					...
					$existing = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'invoice', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID ));
	...
	public static function CustomerAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if ($arr = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'customer', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
			$Customer = new QuickBooks_SQL_Object('customer', null, $arr);
			
			if (!(is_numeric($Customer->getD('CreditCardInfo_ExpirationMonth')) and 
				is_numeric($Customer->getD('CreditCardInfo_ExpirationYear')) and 
				checkdate($Customer->getD('CreditCardInfo_ExpirationMonth'), 1, $Customer->getD('CreditCardInfo_ExpirationYear')) ))
			{
	...
	public static function InventoryAdjustmentAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if ($arr = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'inventoryadjustment', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($CustomerMsg = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'customermsg', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	public static function JournalEntryAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		if ($arr = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'journalentry', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Class = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'class', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	public static function DataExtAddRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		$xml = '';
		if ($arr = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'dataext', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
			...
			<QBXML>
				<QBXMLMsgsRq onError="' . QUICKBOOKS_SERVER_SQL_ON_ERROR . '">
					<DataExtAddRq requestID="' . $requestID . '">
						<DataExtAdd>
							<OwnerID>' . $DataExt->getD("OwnerID") . '</OwnerID>
							<DataExtName>' . $DataExt->getD("DataExtName") . '</DataExtName>
					';
			if($DataExt->getD("EntityType") != null)
			{
				$xml .= '<ListDataExtType>' . $DataExt->getD("DataExtType") . '</ListDataExtType>
						 <ListObjRef>
						 	<ListID>' . $DataExt->getD("Entity_ListID") . '</ListID>
						 </ListObjRef>
						 ';
			}
			else
			{
				$base = str_ireplace(array("line", "group"), "", end(explode("_", $DataExt->getD("TxnType"))));
				$xml .= '<TxnDataExtType>' . $base . '</TxnDataExtType>
						';
				if(stripos($DataExt->getD("TxnType"), "line") !== false)
				{
					$table = strtolower($base) . "_" . strtolower($DataExt->getD("TxnType"));
					if($temp = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . $table, array( "TxnLineID" => $DataExt->getD("Txn_TxnID") )))
					{
						$xml .= '<TxnID>' . $temp->getD( $base . '_TxnID' ) . '</TxnID>
								 <TxnLineID>' . $DataExt->getD("Txn_TxnID") . '</TxnLineID>
								';
					}
					else
						return '';
				...
				{
					$xml .= '<TxnID>' . $DataExt->getD("Txn_TxnID") . '</TxnID>
							';
				}
			}
			$xml .= '<DataExtValue>' . $DataExt->getD("DataExtValue") . '</DataExtValue>
				  </DataExtAdd>
				</DataExtAddRq>
		      </QBXMLMsgsRq>
			</QBXML>';
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($ShipMethod = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'shipmethod', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($PaymentMethod = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'paymentmethod', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'account', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'itemdiscount', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'itemfixedasset', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
    ...
    {
    	$Driver = QuickBooks_Driver_Singleton::getInstance();
    	if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'iteminventoryassembly', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
    	{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'iteminventory', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'itemnoninventory', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'itemothercharge', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'itempayment', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'itemsalestax', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'itemservice', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'itemsubtotal', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'employee', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($arr = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'employee', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'estimate', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'purchaseorder', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'receivepayment', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	public static function ReceivePaymentModRequest($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($ReceivePayment = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'receivepayment', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Invoice = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'invoice', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($SalesReceipt = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'salesreceipt', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Invoice = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'creditmemo', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($JobType = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'jobtype', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'salesorder', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'salesrep', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Account = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'salestaxcode', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Customer = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'bill', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Bill = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'bill', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($BillPaymentCheck = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'billpaymentcheck', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($BillPaymentCreditCard = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'billpaymentcreditcard', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Vendor = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'vendor', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Vendor = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'vendorcredit', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	protected static function _AddRequest($type, $Object, $requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $version, $locale, $config = array())
	{
		...
		$class['index'] = 'QuickBooks_QBXML_Schema_Object_' . QuickBooks_Utilities::actionToRequest($action);
		
		QuickBooks_Loader::load($file);
		$schema_object = new $class['index']();
		...
		foreach ($Object->asArray() as $field => $value)	// map the SQL fields back to XML
		{
			...
			else if (!strlen($value))
			{
					...
					strlen($Object->getD($begi . 'Addr1')))
				{
					...
					/* and 
						( strlen($Object->getD($begi . 'City')) or strlen($Object->getD($begi . 'State')) or strlen($Object->getD($begi . 'Country')) or strlen($Object->getD($begi . 'PostalCode')) ))
					{
						continue;
					}
					*/
			...
			if ($locale == QUICKBOOKS_LOCALE_UK and 
				$last == 'State' and 
				!strlen($Object->getD($begi . 'County')))		// UK *County* support instead of states
			{
		...
		{
			$data = QuickBooks_Callbacks_SQL_Callbacks::_queryChildren($children, $Object->getD($children[0]['parent']));
		}
		else
		{
	...
	protected static function _ChildObjectsToXML($type, $action, $children, $parentPath = '')
	{
		...
		foreach ($children as $child)
		{
			...
			if (strpos($child['table'], "linkedtxn") !== false)
			{
				...
				{
					$Node = new QuickBooks_XML_Node("LinkToTxnID", $child['data']->getD("ToTxnID"));
				...
				else if ($schema_object->exists($usePath . 'ApplyCheckToTxnAdd'))
				{
					...
					$Node->setChildDataAt($Node->name() . ' ' . 'TxnID', $child['data']->getD("ToTxnID"));
					$Node->setChildDataAt($Node->name() . ' ' . 'Amount', $child['data']->getD("ToTxnID"));
					$nodes[count($nodes)] = $Node;
					continue;
				}
	...
	{
		$Driver = QuickBooks_Driver_Singleton::getInstance();
		if ($Check = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'check', array( QUICKBOOKS_DRIVER_SQL_FIELD_ID => $ID )))
		{
	...
	*/
	...
	protected static function _updateRelatives($table, $user, $action, $ID, $object, $extra, $callback_config = array(), $deleteDataExt = false, $fullDelete = false)
	{
		...
		$TxnID_or_ListID = $object->getD($update_relatives_map[$table]['id_field']);
	...
	protected static function _deleteChildren($table, $user, $action, $ID, $object, $extra, &$deleted, $callback_config = array(), $deleteDataExt = false, $fullDelete = false)
	{
		...
		{
			$TxnID_or_ListID = $object->getD($delete_children_map[$table]['id_field']);
					...
					$map[1])
				{
					...
					if ((!isset($extra['IsAddResponse']) and !isset($extra['is_add_response'])) or !(count($addMapTest) and $addMapTest[0]) or $map[0] != $addMapTest[0])
					{
						...
						if (is_array($map[1]))
						{
							...
							{
								$multipart[$table_name] = $object->getD($table_name);
							}
						}
						else
						{
							$multipart[ $map[1] ] = $object->getD( $map[1] );
						}
					}
					else
					{
					...
					if ($tmp = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . $table, $multipart ))
					{
						...
						if (empty($extra['AddResponse_OldKey']) and 
							...
							isset($tmp['TxnLineID']))
						{
							//$extra['AddResponse_OldKey'] = $tmp->getD("TxnLineID");
							...
							$callback_config['mode'] != QuickBooks_WebConnector_Server_SQL::MODE_READONLY)
						{
							...
							switch ($callback_config['conflicts'])
							{
								...
								case QuickBooks_WebConnector_Server_SQL::CONFLICT_SQL:
									...
									$tmp_editsequence_update->set('EditSequence', $object->getD('EditSequence'));
						...
						if (empty($extra['is_query_response']) and 					// However, if is_query_response is set this was a forced-update (like when a balance updates, the EditSequence doesn't change but the record *does* need to be updated)
							...
							$tmp['EditSequence'] == $object->getD('EditSequence'))		
						{
							$actually_do_update = false;
							$actually_do_deletechildren = false;
							$actually_do_updaterelatives = false;
					...
					else
					{
						...
						if ($callback_config['mode'] != QuickBooks_WebConnector_Server_SQL::MODE_WRITEONLY)
						{
							...
							if (isset($deleted[$table][QUICKBOOKS_TXNLINEID][$object->getD(QUICKBOOKS_TXNLINEID)][0]))
							{
								$tmp = $deleted[$table][QUICKBOOKS_TXNLINEID][$object->getD(QUICKBOOKS_TXNLINEID)];
								unset($deleted[$table][QUICKBOOKS_TXNLINEID][$object->getD(QUICKBOOKS_TXNLINEID)]);		// Can't use this anymore after it's been used for an INSERT
							...
							if ('' == $object->getD(QUICKBOOKS_DRIVER_SQL_FIELD_USERNAME_ID))
							{
								$object->set(QUICKBOOKS_DRIVER_SQL_FIELD_USERNAME_ID, null);
							}
							
							if ('' == $object->getD(QUICKBOOKS_DRIVER_SQL_FIELD_EXTERNAL_ID))
							{
	...
	protected static function _triggerActions($user, $table, $Object, $action = null)
	{
		...
		switch (strtolower($table))
		{
			case 'receivepayment_appliedtotxn':
				...
				$where = array(
					'TxnID' => $Object->getD('ToTxnID'),
					);
				
				// @todo WARNING WARNING WARNING THIS DOES NOT WORK, I DONT KNOW WHY!
				
				/*
				if ($Object->getD('TxnType') == 'Invoice' and 
					$arr = $Driver->getD(QUICKBOOKS_DRIVER_SQL_PREFIX_SQL . 'invoice', $where))
				{
			...
			case 'invoice':
				...
				$Driver->queueEnqueue($user, QUICKBOOKS_DERIVE_CUSTOMER, null, true, $priority, 
					array( 'ListID' => $Object->getD('Customer_ListID') ) );
				*/
	...
	protected static function _massageInsertRecord($table, &$object)
	{
		...
		{
			$qb_bool = $object->getD($qb_field_boolean, false);
...
<InvoiceQueryRs requestID="SW52b2ljZVF1ZXJ5fGJlZmUwNWJlYjg0NDEyZWU1NjE0MGRhMzMwZjE0ZTdj" statusCode="0" statusSeverity="Info" statusMessage="Status OK" iteratorRemainingCount="0" iteratorID="{6fcbb18b-68cf-4adb-bf40-164ac0bb4ec0}">
	<InvoiceRet>
		...
		<FullName>QuickBooks Invoice</FullName>
		</TemplateRef>
