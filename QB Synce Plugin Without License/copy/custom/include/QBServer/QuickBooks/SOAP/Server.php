...
class QuickBooks_SOAP_Server
{
	...
	{
		$class['index'] = 'QuickBooks_WebConnector_Request_' . ucfirst(strtolower($request));
	...
	public function handle($raw_http_input)
	{
		...
		if ($Doc = $Parser->parse($errnum, $errmsg))
		{
			...
			{
				$temp['index'] = $method;
				$Response = $this->_class->$temp['index']($Request);
			}
	...
	{
		$temp['index'] = $class;
		$this->_class = new $temp['index']($dsn_or_conn, $map, $onerror, $hooks, $log_level, $raw_http_input, $handler_options, $driver_options, $callback_options);
	}
