...
abstract class QuickBooks_QBXML_Object
{
	...
	public function getTimeCreated($format = null)
	{
		if (!is_null($format))
		{
			return date($format, strtotime($this->getD('TimeCreated')));
		}
		
		return $this->getD('TimeCreated');
	}
	...
	public function getTimeModified($format = null)
	{
		if (!is_null($format))
		{
			return date($format, strtotime($this->getD('TimeModified')));
		}
		
		return $this->getD('TimeModified');
	}
	 ...
	 */
	public function getD($key, $default = null)
	{
	...
	{
		$fullname = $this->getD($fullname_key);
		if (!$fullname)
		{
			$name = $this->getD($name_key);
			$parent = $this->getD($parent_key);
	...
	public function setFullNameType($fullname_key, $name_key, $parent_key, $value)
	{
		if (false !== strpos($value, ':'))
		{
			if ($name_key and $parent_key)
			{
				...
				$value = $this->getD($parent_key) . ':' . $this->getD($name_key);
			}
			else
			{
	...
	public function getDateType($key, $format = 'Y-m-d')
	{
		...
		if ($this->exists($key) and $this->getD($key))
		{
			return date($format, strtotime($this->getD($key)));
		}
	...
	{
		return sprintf('%01.2f', (float) $this->getD($key));
	}
	...
	protected function _schema($request)
	{
		...
		$class['index'] = 'QuickBooks_QBXML_Schema_Object_' . $request;
		$file = 'QuickBooks/QBXML/Schema/Object/' . $request . '.php';
		
		include_once $file;
		
		if (class_exists($class['index']))
		{
