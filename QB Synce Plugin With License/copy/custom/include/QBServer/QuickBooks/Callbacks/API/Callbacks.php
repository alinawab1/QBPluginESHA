...
class QuickBooks_Callbacks_API_Callbacks
{
	...
	protected static function _mapCreate($func, $user, $type, $ID, $ListID_or_TxnID, $editsequence = '', $extra = null)
	{
		if (strlen($func))
		{
			...
			{
				$temp['func'] = $func;
				return $temp['func']($type, $ID, $ListID_or_TxnID, $editsequence, $extra); 
			}
			else
			{
				$tmp = explode('::', $func);
				$class['index'] = new $tmp[0]();
				
				return $class['index']->$tmp[1]($type, $ID, $ListID_or_TxnID, $editsequence, $extra);
	...
	protected static function _mapToQuickBooksID($func, $user, $type, $ID)
	{
		if (strlen($func))
		{
			...
			else
			{
				...
				return $class['index']->$tmp[1]($type, $ID);
//				return call_user_func(array( $tmp[0], $tmp[1] ), $type, $ID);
	...
	static protected function _mapToApplicationID($func, $user, $type, $ListID_or_TxnID)
	{
		if (strlen($func))
		{
			...
			{
				$temp['func'] = $func;
				return $temp['func']($type, $ListID_or_TxnID); 
			}
			else
			{
				$tmp = explode('::', $func);
				...
				return $class['index']->$tmp[1]($type, $ListID_or_TxnID);
