...
class QuickBooks_XML_Parser
{
	...
	public function __construct($xml_or_file = null, $use_backend = null)
	{
		...
		$class['index'] = 'QuickBooks_XML_Backend_' . ucfirst(strtolower($use_backend));
		$this->_backend = new $class['index']($xml_or_file);
	}
...
//			while ($tmp = fread($mixed, 8192))
			while ($tmp = stream_get_contents($mixed, 8192))
			{
				$buffer .= $tmp;
			}
		...
		{
			return sugar_file_get_contents($mixed);
		}
